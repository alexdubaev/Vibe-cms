...
describe('FilesystemPrivateStorage signed URLs', () => {
  ...
  test('stores generated bytes once and leaves the first content immutable', async () => {
    await withSetup(async ({ storage }) => {
      const first = await storage.putObjectOnce('cms/publications/4/snapshot.json', new TextEncoder().encode('first'), 'application/json')
      const second = await storage.putObjectOnce('cms/publications/4/snapshot.json', new TextEncoder().encode('second'), 'application/json')

      expect(first.stored).toBe(true)
      expect(second).toEqual({ stored: false, reason: 'already_exists' })
      expect(await storage.headObject('cms/publications/4/snapshot.json')).toMatchObject({ contentLength: 5, contentType: 'application/json' })
    })
  })
})
