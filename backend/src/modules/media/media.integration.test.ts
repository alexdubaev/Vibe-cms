...
maybeDescribe('media repository and service against PostgreSQL', () => {
  ...
  beforeAll(() => {
    ...
    const storage: PrivateStorage = {
      ...
      readRange: async () => Uint8Array.from([0x89, 0x50, 0x4e, 0x47, 0x0d, 0x0a, 0x1a, 0x0a, ...new Array(24).fill(0)]),
  ...
  test('finalizes bytes against storage and deletes an unused asset by durable handoff', async () => {
    ...
    expect((await db.cmsMediaAsset.findUniqueOrThrow({ where: { id: created.asset.id } })).storageEtag).toBe('etag-1')
