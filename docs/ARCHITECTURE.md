...
- `src/scheduler.ts` is the long-running timer process; `src/worker.ts` is the long-running loop process. The scheduler ships with one entry, `outbox:drain` every minute, and is deployable as shipped; the worker ships empty and deployment specs refuse it until it has entries. See [BACKGROUND_JOBS.md](BACKGROUND_JOBS.md).
...
A closed set of values belongs in a Postgres enum; an open one does not. `task_outbox.status` is an enum because a row is only ever pending, processing, done, skipped or failed, and changing that genuinely is a schema change. `task_outbox.type` is a plain text column validated in code against the handler registry, so adding a task type stays a code change instead of an `ALTER TYPE` that cannot be rolled back. The same reasoning already governs the task named by a scheduled DigitalOcean component, which `scripts/deploy-do.mjs` validates against `backgroundJobNames()` before deploying.
