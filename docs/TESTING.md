...
Hosted validation lives in `.github/workflows/infrastructure-and-backend.yml`: it runs the backend
integration suite on Linux and credential-free Terraform validation. It does not replace local
contracts, webapp, website, builder, E2E, or visual checks. Run the relevant checks locally after
each task, starting with the smallest set that meaningfully covers the changed surface and expanding
only when risk justifies it. Production releases and activated SSG rebuilds follow the selected
hosting provider's deployment runbook; they do not run task validation.
