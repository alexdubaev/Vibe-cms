...
export type S3PublicationStorageOptions = {
  endpoint: string
  bucket: string
  accessKeyId: string
  secretAccessKey: string
  region?: string
  /** Uses endpoint/bucket/key URLs instead of bucket.endpoint/key URLs. Defaults to true for compatibility. */
...
export type PublicationObjectReader = {
  headObject(key: string): Promise<boolean>
  readObject(key: string): Promise<Uint8Array | null>
}

export type S3PublicationStorageAdapter = StaticUploadPort & MediaCopyPort & PublicationObjectReader

export type S3PublicationStorageEnvironment = Record<string, string | undefined>
...
const STORAGE_ENVIRONMENT_NAMES = [
  ...
  'CMS_WEBSITE_STORAGE_FORCE_PATH_STYLE',
] as const
...
/** Parses the complete builder storage group once so startup cannot ignore partial S3 settings. */
export function s3PublicationStorageOptionsFromEnvironment(
  environment: S3PublicationStorageEnvironment,
): S3PublicationStorageOptions {
  const values = Object.fromEntries(STORAGE_ENVIRONMENT_NAMES.map((name) => [name, environment[name]?.trim() ?? ''])) as Record<typeof STORAGE_ENVIRONMENT_NAMES[number], string>
  const missing = STORAGE_ENVIRONMENT_NAMES.filter((name) => !values[name])
  if (missing.length > 0) {
    throw new Error(`Required website builder environment is missing: ${missing.join(', ')}`)
  }
  if (values.CMS_WEBSITE_STORAGE_FORCE_PATH_STYLE !== 'true' && values.CMS_WEBSITE_STORAGE_FORCE_PATH_STYLE !== 'false') {
    throw new Error('CMS_WEBSITE_STORAGE_FORCE_PATH_STYLE must be true or false')
  }

  return {
    endpoint: values.CMS_WEBSITE_STORAGE_ENDPOINT,
    bucket: values.CMS_WEBSITE_STORAGE_BUCKET,
    accessKeyId: values.CMS_WEBSITE_STORAGE_ACCESS_KEY_ID,
    secretAccessKey: values.CMS_WEBSITE_STORAGE_SECRET_ACCESS_KEY,
    region: values.CMS_WEBSITE_STORAGE_REGION,
    forcePathStyle: values.CMS_WEBSITE_STORAGE_FORCE_PATH_STYLE === 'true',
  }
}

/**
 * Provider-neutral S3-compatible storage adapter for a builder release.
 *
 * No object key is ever accepted outside a configured slot (for static releases)
 * or the narrow media destination form.
 */
export function createS3PublicationStorageAdapter(options: S3PublicationStorageOptions): S3PublicationStorageAdapter {
  const endpoint = new URL(options.endpoint)
  if (endpoint.protocol !== 'https:' && endpoint.protocol !== 'http:') {
    throw new Error('S3 publication storage endpoint must use HTTP(S)')
  }
  if (!options.bucket || /[\\/]/.test(options.bucket)) throw new Error('S3 publication storage bucket is invalid')
  if (!options.accessKeyId) throw new Error('S3 publication storage access key is required')
  if (!options.secretAccessKey) throw new Error('S3 publication storage secret key is required')

  const region = options.region ?? DEFAULT_REGION
  if (!region.trim()) throw new Error('S3 publication storage region is required')
  const forcePathStyle = options.forcePathStyle ?? true
  if (typeof forcePathStyle !== 'boolean') throw new Error('S3 publication storage forcePathStyle must be a boolean')
  if (!forcePathStyle && !isVirtualHostCompatibleBucket(options.bucket)) {
    throw new Error('S3 publication storage bucket must be DNS-compatible for virtual-host addressing')
  }
  ...
  async function request(input: {
    ...
    const url = objectUrl(endpoint, options.bucket, forcePathStyle, input.key, input.query)
...
function objectUrl(endpoint: URL, bucket: string, forcePathStyle: boolean, key?: string, query?: Array<[string, string]>) {
  const url = new URL(endpoint.toString())
  const basePath = url.pathname.replace(/\/$/, '')
  if (!forcePathStyle) url.hostname = `${bucket}.${url.hostname}`
  const path = [
    basePath,
    ...(forcePathStyle ? [encodePathSegment(bucket)] : []),
    ...(key === undefined ? [] : key.split('/').map(encodePathSegment)),
  ]
...
function isVirtualHostCompatibleBucket(bucket: string) {
  return /^(?=.{3,63}$)(?!\d+\.\d+\.\d+\.\d+$)[a-z0-9](?:[a-z0-9-]*[a-z0-9])?(?:\.[a-z0-9](?:[a-z0-9-]*[a-z0-9])?)*$/.test(bucket)
}
